...
Lifetime (in seconds) of the temporary data created before authentication completes (state, nonce, PKCE code_verifier, and the original URL) in the server-side store. Specify a value between `1` and `3600` seconds (1 hour); values outside this range are rejected. `0` is rejected because the entries would expire within the same second they are stored, making every legitimate login fail (self-inflicted DoS), and the upper bound caps the CSRF protection window (the state / nonce reuse window). Time strings (e.g., `5m`, `10m`) are also accepted. It controls the grace period between redirecting to the authorization endpoint and the callback returning. The Max-Age of the temporary cookie (`NGX_OIDC_SESSION_CALLBACK`) that correlates the callback with those server-side entries also follows this setting, so a login that completes within `pre_auth_timeout` is not broken by the cookie expiring early. The authorization-code replay guard and the lifetime of the completed session itself (`session_timeout`), on the other hand, are separate concerns and are not affected by this setting. The replay guard uses a fixed internal value (600 seconds).
